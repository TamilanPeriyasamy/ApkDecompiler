...
public class ApkManager {
	...
	public String mAndroidAAPTPath = null;
	...
	public ApkManager() {
		...
		mApkExtractPath  = ApkDecompiler.mApkBuildDir.getAbsolutePath();
		mApkBuildDir     = ApkDecompiler.mApkBuildDir;
		mOutputDir       = ApkDecompiler.mOutputDirectory;
		mAndroidAAPTPath = ApkDecompiler.mAndroidAAPTPath;
		zipalign         = ApkDecompiler.mZipalign;
		apkSigner        = ApkDecompiler.mApkSigner;
		KeyStorePath    = ApkDecompiler.mBuildToolsPath+"/build-tools/key-store.jks";
		outputApkPath    = mOutputDir+File.separator+mApkFile.getName();
		debugApkPath     = mOutputDir+File.separator+"debug-"+mApkFile.getName();
	}


	public static void cleanAppDir() throws IOException {
		FileUtils.cleanDirectory(new File(ApkDecompiler.mUserDir+"/build/"));
		FileUtils.cleanDirectory(new File(ApkDecompiler.mUserDir+"/output/"));
	}
	...
	public void extractingApkFiles() throws Exception {
		...
		ZipFile zipFile = new ZipFile(mApkFile);
	...
	public void buildApkFiles() throws Exception {
		System.out.println("\nBuild apk ...");
		String unsingApkFilePath=mOutputDir.getAbsolutePath()+File.separator+mApkFile.getName();
		...
		String runcommand=mAndroidAAPTPath+" p -f -F "+unsingApkFilePath+" -I "+ApkDecompiler.mAndroidJarPath+" -S "+mApkBuildDir.getAbsolutePath()+"/res "+" -M "+mApkBuildDir.getAbsolutePath()+"/AndroidManifest.xml";
		...
		for(int fileCount=0;fileCount<mRawFileList.size();fileCount++) {
			...
			String runcommand1 =mAndroidAAPTPath+" add -v "+mApkFile.getName()+" \""+filePath+"\"";
		...
		for (String fileNme : listofFiles) {
			if( new File(mOutputDir+File.separator+fileNme).exists()) {
				...
				if(currentFile.isDirectory()) {
					...
					if(currentFile.isFile() && !currentFile.getName().equals(mApkFile.getName())) {
						currentFile.delete();
					}
				}
			}
